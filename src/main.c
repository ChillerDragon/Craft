...
int hit_test_face(Player *player, int *x, int *y, int *z, int *face) {
  State *s = &player->state;
  int w = hit_test(0, s->x, s->y + PLAYER_CAMERA_HEIGHT_OFFSET, s->z, s->rx,
                   s->ry, x, y, z);
  if (is_obstacle(w)) {
    int hx, hy, hz;
    hit_test(1, s->x, s->y + PLAYER_CAMERA_HEIGHT_OFFSET, s->z, s->rx, s->ry,
             &hx, &hy, &hz);
    int dx = hx - *x;
    int dy = hy - *y;
    int dz = hz - *z;
...
void login() {
  ...
  if (db_auth_get_selected(username, sizeof(username), identity_token,
                           sizeof(identity_token))) {
...
void handle_movement(double dt) {
  ...
  if (!g->typing) {
    ...
    g->fov = glfwGetKey(g->window, CRAFT_KEY_ZOOM) ? 15 : 91;
...
int main(int argc, char **argv) {
  ...
  load_png_texture("data/textures/texture.png");
  ...
  load_png_texture("data/textures/font.png");
  ...
  load_png_texture("data/textures/sky.png");
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
  load_png_texture("data/textures/sign.png");
  ...
  program = load_program("data/shaders/block_vertex.glsl",
  ...
  program = load_program("data/shaders/line_vertex.glsl",
                         "data/shaders/line_fragment.glsl");
  line_attrib.program = program;
  line_attrib.position = glGetAttribLocation(program, "position");
  line_attrib.matrix = glGetUniformLocation(program, "matrix");

  program = load_program("data/shaders/text_vertex.glsl",
  ...
  program = load_program("data/shaders/sky_vertex.glsl",
                         "data/shaders/sky_fragment.glsl");
    ...
    while (1) {
      ...
      if (SHOW_INFO_TEXT) {
        ...
        snprintf(text_buffer, sizeof(text_buffer),
                 ...
                 chunked(s->x), chunked(s->z), s->x,
                 s->y + PLAYER_CAMERA_HEIGHT_OFFSET, s->z, g->player_count,
                 g->chunk_count, face_count * 2, hour, am_pm, fps.fps);
